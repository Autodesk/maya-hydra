    ...
    const MHWRender::MDataServerOperation::MViewportScene& scene)
{
    ...
    if (framecontext){
        ...
        if (false == manager.ModelPanelIsAlreadyRegistered(panelName.asChar())){
            ...
            manager.AddViewportInformation(hydraViewportInformation, _renderIndexProxy, _lastFilteringSceneIndexOfTheChainBeforeCustomFiltering);
        }
    }
...
void MtohRenderOverride::_CreateSceneIndicesChainAfterMergingSceneIndex()
{
    ...
    _lastFilteringSceneIndexOfTheChainBeforeCustomFiltering = _renderIndexProxy->GetMergingSceneIndex();

    _selection = std::make_shared<Fvp::Selection>();
    _selectionSceneIndex = Fvp::SelectionSceneIndex::New(_lastFilteringSceneIndexOfTheChainBeforeCustomFiltering , _selection);
    _selectionSceneIndex->SetDisplayName("Flow Viewport Selection Scene Index");
    _lastFilteringSceneIndexOfTheChainBeforeCustomFiltering  = _selectionSceneIndex;
  
    if (!_sceneIndexRegistry) {
        _sceneIndexRegistry.reset(new MayaHydraSceneIndexRegistry(_renderIndexProxy));
    }

    auto wfSi = TfDynamic_cast<Fvp::WireframeSelectionHighlightSceneIndexRefPtr>(Fvp::WireframeSelectionHighlightSceneIndex::New(_lastFilteringSceneIndexOfTheChainBeforeCustomFiltering, _selection));
    ...
    _lastFilteringSceneIndexOfTheChainBeforeCustomFiltering  = wfSi;
