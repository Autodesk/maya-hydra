...
private:
    ...
    /// Whether the pixel outline is the selection-highlight mechanism for this override.
    ///
    /// The outline tasks are only ever installed on frame pass 0 of a Storm pass (see
    /// _InitHydraResources), so the render global alone is not enough: with a non-Storm
    /// delegate the Outline mode cannot be honored and the legacy wireframe highlighting has
    /// to stay in charge. Every site that decides "does the outline own the highlight?" must
    /// go through this, or the legacy highlight gets suppressed with nothing replacing it.
    bool _UseOutlineSelectionHighlighting() const;

    /// Whether nothing may draw a legacy (wireframe) selection highlight: either the pixel
    /// outline owns it, or selection highlighting is disabled outright (test/diagnostic floor).
    ...
    // SetInputs / SetStyle must all be called from the thread driving the frame pass commit, with no
    // internal synchronization. Observers (selection, color preferences, the hover event filter) may
    // therefore only flag work here; the manager itself is touched exclusively from Render().
    ...
    /// Viewport hover state, kept **per panel**.
    ///
    /// Hover is inherently per-viewport -- the cursor is in exactly one panel -- but a single
    /// MtohRenderOverride serves every panel that uses the renderer, and so do _outline and the frame
    /// passes. Holding this state on the override therefore made hovering in one viewport highlight the
    /// same object in all of them, and left the highlight behind when the cursor moved away.
    ///
    /// The Qt event filter records the cursor for its own panel; Render() reads the entry for the panel
    /// it is currently drawing (_currentPanelName). Coordinates are device pixels with Qt's top-left
    /// origin, or -1 for "no hover"; _ResolveHoverPath() flips them to Maya's convention.
    struct HoverState
    {
        ...
        /// signal it, so Render() compares against this to dirty the hover. Per panel because each has
        /// its own camera; a single shared matrix thrashed between them. Render thread only.
        MMatrix lastViewProjMatrix;
    };

    /// Keyed by panel name. Entries are created and destroyed alongside the hover event filter, both on
    /// the main thread, so the map structure is never mutated concurrently; the per-field atomics guard
    /// the values themselves.
    std::map<std::string, std::unique_ptr<HoverState>> _hoverStates;

    /// The panel currently being drawn, recorded by setup() and read by Render(). Maya calls setup()
    /// with the destination panel before each of that panel's renders.
    MString _currentPanelName;

    /// The hover state for \p panelName, or nullptr when that panel has none.
    HoverState* _GetHoverState(const MString& panelName);
    ...
    // One hover event filter per model panel, keyed by panel name. Guarded because
    // Qt is only available when the Maya devkit ships it (see mayaPlugin CMakeLists).
    std::map<std::string, std::unique_ptr<MAYAHYDRA_NS_DEF::MhHoverEventFilter>>
        _hoverEventFilters;
#endif
    ...
    void _SetHoverPosition(const MString& panelName, int deviceX, int deviceY, bool active);
