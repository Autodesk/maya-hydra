#include "piPrototypeWireframeHighlightSi.h"
...
HdSceneIndexBaseRefPtr PiPrototypeWireframeHighlightSceneIndex::New(
    const HdSceneIndexBaseRefPtr& inputSceneIndex,
    const SdfPath& prototypePrimPath,
    const size_t selectionIndex,
    const std::shared_ptr<WireframeColorInterface>& wireframeColorInterface)
{
    return TfCreateRefPtr(new PiPrototypeWireframeHighlightSceneIndex(inputSceneIndex, prototypePrimPath, selectionIndex, wireframeColorInterface));
}

HdSceneIndexPrim PiPrototypeWireframeHighlightSceneIndex::GetPrim(const SdfPath &primPath) const
{
    if (_IsRelevantPath(primPath)) {
        HdSceneIndexPrim prim = GetInputSceneIndex()->GetPrim(primPath);
        if (prim.primType == HdPrimTypeTokens->mesh) {
            // TODO : Actual selection
            prim.dataSource = MakeWireframe(prim.dataSource, _wireframeColorInterface->getWireframeColor(_prototypePrimPath));
        }
        return prim;
    }
    return {};
...
SdfPathVector PiPrototypeWireframeHighlightSceneIndex::GetChildPrimPaths(const SdfPath &primPath) const
{
...
PiPrototypeWireframeHighlightSceneIndex::PiPrototypeWireframeHighlightSceneIndex(
    const HdSceneIndexBaseRefPtr& inputSceneIndex,
    const SdfPath& prototypePrimPath,
    const size_t selectionIndex,
    const std::shared_ptr<WireframeColorInterface>& wireframeColorInterface
    ...
    _prototypePrimPath(prototypePrimPath),
    _selectionIndex(selectionIndex),
    _wireframeColorInterface(wireframeColorInterface)
{
    _CollectInstancingPaths(prototypePrimPath, SelectionHighlightsCollectionDirection::InstancedBy, _instancerPaths, _prototypePaths);
...
void PiPrototypeWireframeHighlightSceneIndex::_PrimsAdded(
...
void PiPrototypeWireframeHighlightSceneIndex::_PrimsRemoved(
    const HdSceneIndexBase &sender,
...
void PiPrototypeWireframeHighlightSceneIndex::_PrimsDirtied(
    const HdSceneIndexBase &sender,
...
bool PiPrototypeWireframeHighlightSceneIndex::_IsInstancerPath(const PXR_NS::SdfPath& primPath) const
{
...
bool PiPrototypeWireframeHighlightSceneIndex::_IsPrototypePath(const PXR_NS::SdfPath& primPath) const
{
    for (const auto& prototypePath : _prototypePaths) {
...
bool PiPrototypeWireframeHighlightSceneIndex::_IsRelevantPath(const PXR_NS::SdfPath& primPath) const
{
    return _IsInstancerPath(primPath) || _IsPrototypePath(primPath);
}

void
PiPrototypeWireframeHighlightSceneIndex::_CollectInstancingPaths(const PXR_NS::SdfPath& primPath, SelectionHighlightsCollectionDirection direction, PXR_NS::SdfPathSet& outInstancerPaths, PXR_NS::SdfPathSet& outPrototypePaths) const
{
    HdSceneIndexPrim prim = GetInputSceneIndex()->GetPrim(primPath);
...
void
PiPrototypeWireframeHighlightSceneIndex::_ForEachPrimInHierarchy(
