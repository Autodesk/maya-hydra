...
class TestFlowViewportAPI(mtohUtils.MayaHydraBaseTestCase): #Subclassing mtohUtils.MayaHydraBaseTestCase to be able to call self.assertSnapshotClose
    ...
    def test_HierarchicalProperties(self):
        with PluginLoaded('mayaHydraFlowViewportAPILocator'):
            self.setBasicCam(10)
            ...
            locator = cmds.createNode("MhFlowViewportAPILocator", parent=locatorParent)
            self.assertSnapshotClose("hierarchicalProperties_locator_visibility_off.png", self.IMAGE_DIFF_FAIL_THRESHOLD, self.IMAGE_DIFF_FAIL_PERCENT)
            # Check that updating a parent's visibility works after creation
            cmds.setAttr(locatorGrandParent + ".visibility", True)
            self.assertSnapshotClose("hierarchicalProperties_locator_visibility_on.png", self.IMAGE_DIFF_FAIL_THRESHOLD, self.IMAGE_DIFF_FAIL_PERCENT)
            # Check that changing the visibility on the shape itself works
            cmds.setAttr(locator + ".visibility", False)
            self.assertSnapshotClose("hierarchicalProperties_locator_visibility_off.png", self.IMAGE_DIFF_FAIL_THRESHOLD, self.IMAGE_DIFF_FAIL_PERCENT)
            
            cmds.delete(sphereShape)
            cmds.delete(locator)
            ...
            # Hide/unhide parent
            self.assertSnapshotClose("hierarchicalProperties_usdStage_visibility_on.png", self.IMAGE_DIFF_FAIL_THRESHOLD, self.IMAGE_DIFF_FAIL_PERCENT)
            cmds.setAttr(stageParent + ".visibility", False)
            self.assertSnapshotClose("hierarchicalProperties_usdStage_visibility_off.png", self.IMAGE_DIFF_FAIL_THRESHOLD, self.IMAGE_DIFF_FAIL_PERCENT)
            ...
            self.assertSnapshotClose("hierarchicalProperties_usdStage_parentTransformChanged.png", self.IMAGE_DIFF_FAIL_THRESHOLD, self.IMAGE_DIFF_FAIL_PERCENT)
            ...
            self.assertSnapshotClose("hierarchicalProperties_usdStage_shapeTransformChanged.png", self.IMAGE_DIFF_FAIL_THRESHOLD, self.IMAGE_DIFF_FAIL_PERCENT)
