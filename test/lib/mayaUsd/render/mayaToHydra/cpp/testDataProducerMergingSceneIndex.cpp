...
TEST(FlowViewport, dataProducerMergingSceneIndex)
{
    // The data producer merging scene index is in the scene index tree.
    ...
    // The data producer merging scene index has the MayaHydraSceneIndex as a
    // child, to produce Maya Dag data.
    ...
    // Ensure that data producers input to the data producer merging scene
    // index have registered a path mapper, to translate an application scene
    // path into a scene index SdfPath.
    ...
    // The Maya data producer scene index has registered a path mapper.  Ask it
    // to translate the application path into a scene index path.
    auto sceneIndexPath = Fvp::sceneIndexPath(mayaPath);
...
TEST(FlowViewport, dataProducerMergingSceneIndexAddRemove)
{
    // Same setup as dataProducerMergingSceneIndex test.
    const auto& sceneIndices = GetTerminalSceneIndices();
    auto isDataProducerMergingSceneIndex = SceneIndexDisplayNamePred(
        "Data Producer Merging Scene Index");
    auto mergingSiBase = findSceneIndexInTree(
    ...
    // With Maya scene index in the data producer merging scene index, the
    // sphere prim has a valid scene index path.
    auto spherePrim = sceneIndices.front()->GetPrim(sceneIndexPath);
    ASSERT_TRUE(spherePrim.dataSource);
    ...
    // Add the Maya scene index back to the data producer merging scene index,
